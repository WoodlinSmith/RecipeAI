...
{
	int x, y;
	int minDist = map.width * map.height + 1;
	direction ret = STAY;
	...
	for ( x = 0; x < map.width; ++x )
	{
		for ( y = 0; y < map.height; ++y )
		{
			...
			if ( map.map[x + y * map.width] > 0 &&
			        ...
			        calcDist ( status.game_x, status.game_y, x, y ) < minDist &&
			        calcDist ( status.game_x, status.game_y, x, y ) > 1 )
			{
				minDist = calcDist ( status.game_x, status.game_y, x, y );

				if ( abs ( status.game_x - x ) > abs ( status.game_y - y ) ) //more horizontal distance
				{
					if ( status.game_x > x )
					{
	...
	if ( xPos < ( mWidth - offset ) && yPos < ( mHeight - offset ) && ret == STAY )
	{
		ret = DOWNRIGHT;
	}
	...
	if ( xPos < ( mWidth - offset ) && ret == STAY )
	{
		ret = RIGHT;
	}

	if ( yPos > ( mHeight + offset ) && ret == STAY )
	{
		ret = UP;
	}
	...
	for ( x = 0; x < map.width; ++x )
		for ( y = 0; y < map.height; ++y )
			...
			if ( map.map[x + y * map.width] //if there is an actor at X, Y
			        ...
			        && calcDist ( status.game_x, status.game_y, x, y ) < minDist ) //And it is the closest one
